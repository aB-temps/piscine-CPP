...
// VECTOR --------------------
void	PmergeMe::_binaryInsert(std::vector<uint32_t> &dest, std::vector<uint32_t>::const_iterator src, std::vector<uint32_t>::iterator maxI,  const uint32_t elemSize)
{
	const uint32_t	element = *(src + elemSize - 1);
	std::vector<uint32_t>::iterator minI = dest.begin() + elemSize - 1;
	std::vector<uint32_t>::iterator midI;
	while (minI <= maxI)
	{
		midI = minI + roundToNextElemSize(((maxI - minI) >> 1), elemSize);
#ifdef DEBUG
...
void	displayElem(std::vector<uint32_t>::iterator it, const uint32_t size)
{
	cout << BOLD "[" RST GREEN;
	for (uint32_t i = 0; i < size; ++i)
		cout << *(it + i) << (i == size - 1 ? "" : " ");
...
void	displayElemFull(std::vector<uint32_t> &vec, const uint32_t elemSize)
{
	std::vector<uint32_t>::iterator it;

	if (!vec.size())
		cout << YELLOW "empty" << RST;
	for (it = vec.begin(); it <= vec.end() - elemSize; it += elemSize)
		displayElem(it, elemSize);
}

void	PmergeMe::_recursivePairSorting(std::vector<uint32_t> &base)
{
	static uint32_t u = 0;
#ifdef DEBUG
	cout << "-----------------\nTurn no " BLUE BOLD << u + 1 << RST ":\n";
#endif
	std::vector<uint32_t>::iterator it = base.begin();
...
std::vector<uint32_t>::iterator	determineBinInsertBound(std::vector<uint32_t> &vec, const uint32_t value)
{
	return (std::find(vec.begin(), vec.end(), value));
}

void	PmergeMe::_unrollingInsertion(std::vector<uint32_t> &base, uint32_t &depth)
{
#ifdef DEBUG
	cout	<< "==> Entering via depth: " BOLD GREEN << depth << RST "\nwith base: ";
#endif
	...
	if (pend.size())
	{
		...
		while (inserted < pend.size() / elemSize)
		{
			...
			while (insertI > lastJsVal)
			{
				...
				// defining maxBound
				std::vector<uint32_t>::iterator	maxBoundIt = main.end() - 1;
				const uint32_t	winnerPos = (((elemSize << 1) * insertI) - 1);
				if (winnerPos < base.size())
				{
					const uint32_t	winnerVal = base[winnerPos];
